...
//
// Отдаёт ряды для главной страницы, собранные из коллекций Кинопоиска
...
// чтобы на фронте не появилось второго пути разбора данных.
//
...
// Ряд главной показывает 20 карточек — ровно страница Кинопоиска.
var ROW_LIMIT = 20;
// Столько же, сколько COLLECTION_CACHE_TTL_MS у серверных подборок.
var TTL_MS = 3 * 24 * 60 * 60 * 1000;
// Одновременных запросов к TMDB при сборке. Больше не нужно: сборка редкая,
// а зеркало общее с остальным сервером.
var TMDB_CONCURRENCY = 4;
var FETCH_TIMEOUT_MS = 15000;

// Все типы коллекций, которые реально отвечает API (проверено перебором;
// TOP_COMMUNITY, TOP_AWAIT, TOP_100_POPULAR_FILMS, TOP_250_BEST_FILMS — не
// существуют, отдают ошибку конвертации типа).
// В скобках — сколько всего элементов в коллекции; в ряд берутся первые ROW_LIMIT.
var COLLECTIONS = {
    kp_popular:    { type: 'TOP_POPULAR_ALL',      name: 'Кинопоиск · Популярное' },          // 700
    kp_pop_movies: { type: 'TOP_POPULAR_MOVIES',   name: 'Кинопоиск · Популярные фильмы' },   // 700
    kp_pop_series: { type: 'POPULAR_SERIES',       name: 'Кинопоиск · Популярные сериалы' },  // 700
    kp_top250:     { type: 'TOP_250_MOVIES',       name: 'Кинопоиск · Топ 250 фильмов' },     // 250
    kp_top250_tv:  { type: 'TOP_250_TV_SHOWS',     name: 'Кинопоиск · Топ 250 сериалов' },    // 250
    kp_releases:   { type: 'CLOSES_RELEASES',      name: 'Кинопоиск · Скоро в кино' },        // 7
    kp_family:     { type: 'FAMILY',               name: 'Кинопоиск · Семейное' },            // 230
    kp_comics:     { type: 'COMICS_THEME',         name: 'Кинопоиск · Комиксы' },             // 100
    kp_love:       { type: 'LOVE_THEME',           name: 'Кинопоиск · Про любовь' },          // 70
    ...
    kp_oscar:      { type: 'OSKAR_WINNERS_2021',   name: 'Кинопоиск · Оскар 2021' }           // 15
};
...
function fetchKpPage(type, page) {
    var url = KP_BASE + '?type=' + encodeURIComponent(type) + '&page=' + (page || 1);
...
function buildCollection(key, log) {
    ...
    var promise = fetchKpPage(cfg.type, 1)
...
module.exports = {
    ...
    init: function (app, ctx) {
        ...
        log.log('Подборки Кинопоиска зарегистрированы: ' + Object.keys(COLLECTIONS).join(', '));
        return { ready: true };
    },
