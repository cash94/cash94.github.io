  ...
  if (backFromDetail) {
    backFromDetail.addEventListener('click', function () {
      ...
      var returnTo = (!AppState || !AppState.isSearch)
        ? ((AppState && AppState.detailReturnTo === 'catalog') ? 'catalog' :
          (AppState && AppState.detailReturnTo === 'local') ? 'local' : 'torrents')
        : 'search';
      ...
      setTimeout(function () {
        ...
        if (returnTo === 'catalog') {
          ...
          if (detailView) detailView.style.display = 'none';
          return;
        } else if (returnTo === 'local') {
          if (typeof window.showLocalCatalog === 'function') {
            window.showLocalCatalog();
            var detailView = document.getElementById('detail-view');
            if (detailView) detailView.style.display = 'none';
            return;
          }
