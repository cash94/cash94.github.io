// torrents.js - Работа с TorrServer и торрентами (Оптимизированная версия)
// Исправлены синтаксические ошибки, добавлен DocumentFragment, кэширование с лимитами, совместимость со старыми Android TV
...
function cacheSet(cache, key, value, max) {
  if (cache.size >= max) { try { var k = cache.keys().next().value; cache.delete(k); } catch (e) { } }
  cache.set(key, value);
}
...
function renderTorrents() {
  ...
  if (AppState.torrents.length === 0) {
    grid.innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:60px 20px"><div style="font-size:48px;margin-bottom:20px">📂</div><div style="color:#aaa">Нет торрентов. Используйте поиск.</div></div>';
    return;
  }
  ...
  for (var i = 0; i < AppState.torrents.length; i++) {
    ...
    attachTorrentDeleteLongPress(card, t);
    (function (torrent) { card.onclick = function () { showDetail(torrent); }; })(t);
...
// Long Press для удаления (сохранен оригинальный механизм)
...
function attachTorrentDeleteLongPress(card, torrent) {
  ...
  var startHold = function (event) {
    ...
    var timer = setTimeout(function () {
      ...
      removeTorrentByHash(torrent.hash, { skipConfirm: true });
  ...
  card.addEventListener('touchend', stopHold); card.addEventListener('touchcancel', stopHold); card.addEventListener('touchmove', stopHold);
  card.addEventListener('mousedown', startHold); card.addEventListener('mouseup', stopHold); card.addEventListener('mouseleave', stopHold);
  card.addEventListener('click', function (e) {
    if (card.dataset.suppressClick === '1' || Date.now() < suppressTorrentClickUntil) { e.preventDefault(); e.stopImmediatePropagation(); e.stopPropagation(); delete card.dataset.suppressClick; return false; }
  }, true);
  card.addEventListener('contextmenu', function (e) {
    e.preventDefault(); suppressTorrentClickUntil = Date.now() + 1200; card.dataset.suppressClick = '1';
    removeTorrentByHash(torrent.hash, { skipConfirm: true }).finally(function () { setTimeout(function () { if (card) delete card.dataset.suppressClick; }, 1200); });
  });
}
...
async function removeTorrentByHash(hash, options) {
  ...
  options = options || {};
  var t = null; for (var i = 0; i < AppState.torrents.length; i++) if ((AppState.torrents[i].hash || '').toLowerCase() === String(hash).toLowerCase()) { t = AppState.torrents[i]; break; }
  if (!options.skipConfirm && !window.confirm('Удалить ' + (t && t.title || 'торрент') + '?')) return false;
  ...
  try {
    ...
    if (AppState.currentDetailItem && (AppState.currentDetailItem.hash || '').toLowerCase() === String(hash).toLowerCase()) { document.getElementById('detail-view').style.display = 'none'; AppState.currentDetailItem = null; AppState.currentScreen = 'torrents'; document.getElementById('torrserver-section').style.display = 'block'; }
...
window.setTorrentClickSuppressed = function (ms) { if (ms === undefined) ms = 1200; suppressTorrentClickUntil = Date.now() + ms; };

// ==================== ДЕТАЛИ И TMDB ====================
function showDetailByHash(hash) {
  if (!hash) return false;
  var h = hash.toLowerCase();
  for (var i = 0; i < AppState.torrents.length; i++) if (AppState.torrents[i].hash && AppState.torrents[i].hash.toLowerCase() === h) { showDetail(AppState.torrents[i]); return true; }
  return false;
}

function hideCatalogDetailExtra() {
  ['catalog-detail-extra', 'files-list', 'detail-subtitle', 'catalog-detail-backdrop', 'catalog-detail-meta', 'catalog-detail-overview', 'catalog-detail-trailers-wrap', 'catalog-detail-trailers', 'catalog-detail-screenshots-wrap', 'catalog-detail-screenshots'].forEach(function (id) {
    var el = document.getElementById(id); if (el) { el.classList.add('hidden'); el.innerHTML = ''; if (id === 'detail-subtitle') el.textContent = ''; if (id === 'catalog-detail-backdrop') el.style.backgroundImage = ''; }
  });
}
...
async function getTmdbDetailsWithCache(id, mt) {
  if (!id) return null; mt = mt || 'movie';
  if (window.getFromTmdbCache && window.saveToTmdbCache) {
    var p = { id: id, type: mt }, c = window.getFromTmdbCache('details', p); if (c) return c;
    try { var r = await fetch('/api/tmdb/details?id=' + id + '&type=' + mt); if (r.ok) { var d = await r.json(); window.saveToTmdbCache('details', p, d); return d; } } catch (e) { }
  }
  if (!window.tmdbDetailsCache) window.tmdbDetailsCache = new Map();
  var k = id + '_' + mt; if (window.tmdbDetailsCache.has(k)) { var c = window.tmdbDetailsCache.get(k); if (Date.now() - c.timestamp < 86400000) return c.data; }
  try { var r = await fetch('/api/tmdb/details?id=' + id + '&type=' + mt); if (r.ok) { var d = await r.json(); window.tmdbDetailsCache.set(k, { data: d, timestamp: Date.now() }); return d; } } catch (e) { } return null;
}

// Файлы с кэшированием
function getTorrentFiles(t) { if (!t) return []; if (t.file_stats && Array.isArray(t.file_stats) && t.file_stats.length) return t.file_stats; if (t.data) try { var d = JSON.parse(t.data); if (d.TorrServer && Array.isArray(d.TorrServer.Files)) return d.TorrServer.Files; } catch (e) { } return []; }
...
function getPreferredPlaybackFile(t, sr) { var vf = getVideoFilesFromTorrent(t); if (!vf.length) return { fileId: 1, episodeIndex: null, isSeries: inferSearchResultIsSeries(sr, t) }; var isS = vrSearchResultIsSeries(sr, t) || vf.length > 1; return { fileId: (vf[0] && vf[0].id) || 1, episodeIndex: isS ? 0 : null, isSeries: isS }; }
...
// ==================== ОСНОВНОЙ ПОКАЗ ДЕТАЛЕЙ ====================
async function showDetail(torrent) {
  window.initHorizontalScroll();
  var dv = document.getElementById('detail-view'); resetDetailBackground();
  if (typeof Animations !== 'undefined') Animations.animateDetailShow();
  dv.style.pointerEvents = 'auto'; var mc = document.getElementById('main-container'); if (mc) mc.style.pointerEvents = 'none';
...
async function loadAllTmdbDataForTorrent(torrent, els) {
  ...
  try { if (torrent.file_stats && torrent.file_stats.length > 1) isTv = true; else if (torrent.data) { var d = JSON.parse(torrent.data); if (d.TorrServer && d.TorrServer.Files && d.TorrServer.Files.length > 1) isTv = true; } } catch (e) { }
...
async function loadProgressForFileItem(item, hash, fid, ep) {
  ...
  try {
    ...
    if (r.ok) { var d = await r.json(); if (d.success && d.timecode > 0 && d.duration > 0) { updateProgressBar(item, d); cacheSet(progressCache, ck, { data: d, timestamp: Date.now() }, MAX_CACHE_SIZE); } }
  } catch (e) { }
}
...
function normalizeSearchResult(item) {
  var n = {}; for (var k in item) if (item.hasOwnProperty(k)) n[k] = item[k];
  var y = parseInt(n.released || n.relased || n.year, 10); n.released = Number.isFinite(y) ? y : null; n.relased = n.released;
  n.tracker = (n.tracker || '').toLowerCase().trim(); return n;
}
...
async function searchTorrentsLegacy(query) {
  ...
  try {
    ...
    if (!r.ok) throw new Error('Ошибка'); searchResults = (await r.json() || []).map(normalizeSearchResult);
...
async function searchTorrents(query) {
  ...
  var m = getCurrentSearchMode(); if (m === 'globalsearch') await searchTMDB(query); else await searchTorrentsLegacy(query);
}
...
function updateAvailableSeasons() {
  ...
  var ss = Object.keys(s).map(Number).sort(function (a, b) { return a - b; });
  sf.innerHTML = '<option value="all">Все</option>'; ss.forEach(function (x) { sf.innerHTML += '<option value="' + x + '"' + (currentSeasonFilter == String(x) ? ' selected' : '') + '>' + x + '</option>'; });
...
function hideSearchResults() {
  ...
  var so = document.getElementById('search-overlay'), st = document.getElementById('tab-search'), tt = document.getElementById('tab-torrents'), ct = document.getElementById('tab-catalog');
  if (so) so.classList.add('hidden'); if (st) st.classList.remove('active'); toggleSearchFiltersPanel(false);
  document.getElementById('torrserver-section').style.display = 'block';
  if (ret === 'detail') {
    AppState.currentScreen = 'detail'; var mc = document.getElementById('main-container'); if (mc && AppState.backupScroll > 0) mc.scrollTop = AppState.backupScroll;
    var dv = document.getElementById('detail-view'); if (dv) { dv.style.display = 'block'; dv.style.pointerEvents = 'auto'; }
    requestAnimationFrame(function () { if (typeof updateFocusableElements === 'function' && typeof setFocus === 'function') { updateFocusableElements(); var wb = document.getElementById('catalog-watch-btn'); if (wb) { var i = -1, fe = window.focusableElements || []; for (var j = 0; j < fe.length; j++) if (fe[j].id === 'catalog-watch-btn') { i = j; break; } if (i !== -1) setFocus(i); } } });
  }
  else if (ret === 'catalog') { if (ct) ct.classList.add('active'); if (tt) tt.classList.remove('active'); AppState.currentScreen = 'catalog'; setTimeout(function () { if (typeof window.focusCatalogCardByIndex === 'function') window.focusCatalogCardByIndex(parseInt(localStorage.getItem('lastCatalogCardIndex') || 0)); else if (typeof window.focusFirstCatalogCard === 'function') window.focusFirstCatalogCard(); }, 80); }
  else { if (tt) tt.classList.add('active'); if (ct) ct.classList.remove('active'); AppState.currentScreen = 'torrents'; setTimeout(function () { if (typeof window.focusFirstTorrentCard === 'function' && !window.focusFirstTorrentCard()) { if (typeof updateFocusableElements === 'function' && typeof setFocus === 'function') { updateFocusableElements(); var c = -1, fe = window.focusableElements || []; for (var j = 0; j < fe.length; j++) if (fe[j].classList && fe[j].classList.contains('torrent-card')) { c = j; break; } if (c !== -1) setFocus(c); } } }, 80); }
...
async function addTorrentToServer(magnet, hash, sr) {
  ...
  try {
    var tn = '[' + (typeof catalogState !== 'undefined' && catalogState.lastSelectedId ? catalogState.lastSelectedId : '') + '] ' + (sr && (sr.name || sr.title) || '');
...
function showContentTypeFilter() {
  var fg = document.querySelector('.filter-group'); if (!fg) return;
  if (!document.getElementById('filter-content-type')) {
    var nf = document.createElement('div'); nf.className = 'filter-group'; nf.innerHTML = '<label class="filter-label" for="filter-content-type">Тип контента</label><select id="filter-content-type" class="filter-select"><option value="all">Все</option><option value="movie">Фильмы</option><option value="tv">Сериалы</option></select>';
    var qf = document.getElementById('filter-quality'); if (qf && qf.parentNode) qf.parentNode.parentNode.insertBefore(nf, qf.parentNode.nextSibling); else fg.parentNode.appendChild(nf);
    document.getElementById('filter-content-type').addEventListener('change', function (e) { filterGlobalSearchByType(e.target.value); });
  }
}
...
function getRatingColor(r) { r = parseFloat(r); return r >= 8 ? '#4caf50' : r >= 6 ? '#ffc107' : r >= 4 ? '#ff9800' : '#f44336'; }
...
function renderFilteredGlobalResults(results) {
  ...
  div.addEventListener('click', function (e) {
    var g = e.target.closest('.global-search-card'); if (g && g.dataset.tmdbId) { AppState.isSearch = true; showGlobalSearchDetail(globalSearchResults.find(function (x) { return String(x.id) === g.dataset.tmdbId; })); }
  });
}
...
function initTorrents() {
  console.log('Torrents module init');
...
function initSearchModeToggle() { var m = document.getElementById('search-mode'); if (m) m.addEventListener('change', function (e) { currentSearchMode = e.target.value; if (currentSearchQuery) searchTorrents(currentSearchQuery); }); }
