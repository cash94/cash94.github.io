...
var CATALOG_CONSTANTS = {
    ...
    // Собирать ряды сразу со всеми постерами: каждый ряд, попав в DOM при
    // сборке главной каталога, уходит в очередь целиком, не дожидаясь, пока
    // въедет в кадр. Экран получается готовым — куда бы человек ни поехал,
    // картинки уже на месте.
    //
    // Плата — вся пачка сразу: примерно 16 рядов по 11 карточек, то есть под
    // две сотни запросов и декодов на старте. Очередь их размазывает
    // (ROW_POSTER_CONCURRENCY плюс пауза на время навигации), но на слабом
    // устройстве это заметная работа в первые секунды. Выключить — false:
    // тогда постеры грузятся как раньше, по мере появления рядов в кадре
    // плюс ROW_POSTER_PRELOAD_ROWS вперёд от фокуса.
...
function dehydrateChunk(ch, gapHint) {
     ...
     *
     * Берём её из measureCatalogCardHeight (catalogState.rowBoxH), а не меряем
     * здесь. Изначально это было вынужденно: при content-visibility: auto
     * getBoundingClientRect у далёкого от кадра чанка отдавал не настоящую
     * высоту, а объявленный резерв contain-intrinsic-size. Свойство из CSS
     * убрали, и это конкретное расхождение ушло — но считать по линейке всё
     * равно правильнее: чтение геометрии стоит здесь посреди цикла с
     * insertBefore и removeChild, то есть ровно там, где синхронная раскладка
     * дороже всего.
     *
     * Опасение «посчитанная разъедется при правке плотности» снимается тем,
     * что и высота строки, и gap пересчитываются на каждую свёртку, а не
     * запоминаются один раз на сессию.
     *
...
/**
 * Что из карточки TMDB можно подсказать Jacred при поиске торрентов.
 *
 * is_serial (1 — фильм, 2 — сериал) отдаём всегда: тип взят из media_type
 * карточки, где он достоверен, а выгода большая — без него в выдачу
 * попадают одноимённые чужие раздачи другого типа.
 *
 * year передаём ТОЛЬКО фильмам, и это не осторожность, а замеры по живому
 * jac.red. Фильму год нужен: «Дюна» без него отдаёт 256 раздач вперемешку
 * (2021, экранизация 1984 и мини-сериал 2000), с year=2021 — ровно 169 своих.
 * А у сериала это год первого сезона, тогда как раздачи поздних сезонов
 * подписаны своими годами, и фильтр их срезает: у «Во все тяжкие» year=2008
 * убирает 21 раздачу — весь 2–5 сезон. То же самое у «Тьмы», «Игры престолов»
 * и «Пацанов».
 *
 * title_original не передаём: он режет гораздо грубее года (на проверках до
 * десятка раз) и выбрасывает раздачи с чисто русским именем.
 */
...
function showCatalogSearch(q, pu, item) {
    ...
    if (st && tt && ct && so) {
        ...
        // Год и тип для Jacred. Ставится после AppState.mediaType выше: он
        ...
        /* Фокус здесь БОЛЬШЕ НЕ СТАВИМ, и это не мелочь.
         *
         * Раньше оверлей поиска на этом месте только терял класс .hidden, а
         * display:none с него снимал уже showSearchResults — то есть вместе с
         * готовыми результатами. Пока он был скрыт, у всего внутри offsetParent
         * был null, фокусировать было нечего, и этот вызов вхолостую проходил
         * мимо.
         *
         * Теперь оверлей проявляется сразу (ради плавного перехода из
         * карточки), и тот же вызов через 200 мс попадает на ПУСТОЙ экран
         * поиска: результатов ещё нет, и фокус садится на строку запроса или на
         * «Фильтры». Через мгновение приезжают результаты, showSearchResults
         * ставит фокус заново — и человек видит, как фокус сначала оказывается
         * не там, а потом сам куда-то переезжает.
         *
         * Фокус ставит showSearchResults, ровно один раз и уже по готовому
         * списку. Пустая выдача тоже проходит через него, так что экран без
         * фокуса не останется. */
        /* Ответ отдаём сразу, без ожидания конца проявления оверлея.
         *
         * Здесь была попытка придержать его до непрозрачности — чтобы
         * detail-view прятался строго под готовым оверлеем. Обернулось
         * зависанием: showSearchResults зовёт fadeIn на том же оверлее второй
         * раз, а fadeIn начинается со stopFade — и onDone первого вызова уже
         * никогда не сработает. Промис не разрешался, detail-view не прятался
         * вовсе, и пульт навсегда оставался у скрытой карточки.
         *
         * Ждать и не нужно: оверлей непрозрачный и появляется мгновенно, а
         * поиск идёт заметно дольше любого перехода. */
        return searching;
    }
...
function loadCatalogRowsProgressively(container, keys) {
    ...
    function observeRowPosters(row) {
        var fresh = false;
        if (!catalogState.rowPosterObserver) {
            // Подписывает все карточки, что уже в DOM, — эту в том числе
            initRowPosterLazyLoading();
            fresh = true;
        }

        // Сборка со всеми постерами: ряд целиком в очередь, наблюдателя не ждём.
        // Порядок постановки = порядок загрузки, а ряды приходят сверху вниз,
        // поэтому верхние картинки появляются первыми — как их и читают.
...
/**
 * Высота строки сетки — линейка для распорок чанковой виртуализации.
 *
 * НЕ УДАЛЯТЬ вместе с остатками content-visibility, хотя выглядит как его
 * обслуга. Появилась она действительно ради contain-intrinsic-size (резерв под
 * неотрисованной карточкой обязан был совпадать с реальной высотой ряда, иначе
 * каждый входящий в кадр ряд толкал всё, что ниже), и CSS-переменную
 * --catalog-card-h сейчас не читает никто: оба правила contain-intrinsic-size
 * закомментированы.
 *
 * Но главный потребитель теперь другой — catalogState.rowBoxH. По нему
 * dehydrateChunk считает высоту распорки, а hydrateChunk — шаг её ужимания.
 * Без замера оба откатываются на getBoundingClientRect, и разворот чанка
 * начинает сдвигать содержимое: это и есть «ступенька» при листании вверх.
 *
 * Реальную высоту знает только отрисованная сетка: она зависит от ширины окна,
 * числа колонок, шрифта и плотности (всё это задаёт ui-customizer.js).
 */
