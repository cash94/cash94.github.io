...
// Проверка поддержки PWA и установки
async function initPWA() {
  ...
  window.addEventListener('beforeinstallprompt', (e) => {
    console.log('🔄 Приложение может быть установлено');
    e.preventDefault();
...
// Функция для кэширования постера
async function cachePoster(posterUrl, torrentId) {
  if (!posterUrl) return;
  
  try {
    const cache = await caches.open('posters-v1');
    const response = await fetch(posterUrl);
    if (response.ok) {
      await cache.put(`/posters/${torrentId}.jpg`, response);
      console.log('Постер сохранен в кэш');
    }
  } catch (error) {
    console.error('Ошибка кэширования постера:', error);
  }
}

// Функция для загрузки постера из кэша
async function getCachedPoster(torrentId) {
  try {
    const cache = await caches.open('posters-v1');
    const response = await cache.match(`/posters/${torrentId}.jpg`);
    if (response) {
      const blob = await response.blob();
      return URL.createObjectURL(blob);
    }
  } catch (error) {
    console.error('Ошибка загрузки постера из кэша:', error);
  }
  return null;
}

// Запуск PWA инициализации
if (document.readyState === 'loading') {
...
// Экспорт PWA функций
window.cachePoster = cachePoster;
