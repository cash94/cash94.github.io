...
function updateFocusableElements() {
    ...
    if (screen === 'player') {
        ...
        var filterControlsList = document.querySelectorAll('#torrent-movie, #sort-by, #filter-quality, #filter-content-type, #filter-tracker, #filter-year, #reset-filters');
        var filterControls = [];
...
function focusSearchHome(preferQuery) {
    ...
    for (var i = 0; i < focusableElements.length; i++) {
        ...
        if (['filter-toggle', 'torrent-movie', 'sort-by', 'filter-quality', 'filter-content-type', 'filter-tracker', 'filter-year', 'reset-filters', 'close-search'].indexOf(el.id) !== -1 && filterIndex === -1) {
            filterIndex = i;
        }
    }
    ...
    if (focusableElements.length === 0) {
        ...
        if (AppState.currentScreen === 'torrents') {
            ...
            for (var l = 0; l < focusableElements.length; l++) {
                if (['filter-toggle', 'torrent-movie', 'sort-by', 'filter-quality', 'filter-content-type', 'filter-tracker', 'filter-year', 'reset-filters', 'close-search'].indexOf(focusableElements[l].id) !== -1) {
                    firstFilterIndex = l;
                    break;
                }
            }
...
function setupFocusRescue() {
    ...
    function getSearchFilters() {
        var ids = ['torrent-movie', 'sort-by', 'filter-quality', 'filter-content-type', 'filter-tracker', 'filter-year', 'reset-filters'];
        var visible = [];
        for (var i = 0; i < ids.length; i++) {
            var el = byId(ids[i]);
            if (VISIBLE(el)) visible.push(el);
    ...
    function belongsToScreen(el, screen) {
        ...
        if (screen === 'search') {
            return el.closest('.search-result-item') || el.closest('.global-search-card') ||
                ['search-query', 'filter-toggle', 'search-btn', 'close-search', 'torrent-movie', 'sort-by', 'filter-quality', 'filter-content-type', 'filter-tracker', 'filter-year', 'reset-filters'].indexOf(el.id) !== -1;
        }
        if (screen === 'detail') {
            return !!(
    ...
    function openFiltersPanelAndFocus() {
        ...
        if (panel && panel.classList.contains('collapsed')) {
            ...
            setTimeout(function () {
                updateFocusableElements();
                var firstFilter = document.querySelector('#torrent-movie, #sort-by, #filter-quality, #filter-content-type, #filter-tracker, #filter-year, #reset-filters');
                if (firstFilter) {
                    var filterIndex = -1;
                    for (var i = 0; i < focusableElements.length; i++) {
