...
function setupFocusRescue() {
    ...
    function detailHandle(dir) {
        ...
        var toTop = function () { var dv = getEl('detail-view'); if (dv) dv.scrollTo({ top: 0, behavior: 'smooth' }); };
        if (isF && fii !== -1) { if (dir === 'left') { if (fii > 0) { focusEl(fi[fii - 1]); } return true; } if (dir === 'right') { if (fii < fi.length - 1) { focusEl(fi[fii + 1]); } return true; } if (dir === 'up') { var prevItems = []; for (var k = idx - 1; k >= 0; k--) { if (!items[k].classList || !items[k].classList.contains('file-item')) { prevItems.push(items[k]); } } if (prevItems.length > 0) { focusEl(prevItems[0]); if (prevItems[0].id === 'catalog-watch-btn' || prevItems[0].id === 'back-from-detail') { toTop(); } } return true; } if (dir === 'down') return true; return true; }
        if (isT && ti !== -1) { if (dir === 'left') return focusEl(tl[Math.max(0, ti - 1)] || f); if (dir === 'right') return focusEl(tl[Math.min(tl.length - 1, ti + 1)] || f); if (dir === 'up') { if (wb && wb.offsetParent !== null) { focusEl(wb); toTop(); return true; } return focusEl(items[Math.max(0, idx - 1)] || f); } if (dir === 'down') { if (ac.length > 0) { focusEl(ac[0]); return true; } else if (rc.length > 0) { focusEl(rc[0]); return true; } else if (fi.length > 0) { focusEl(fi[0]); return true; } return true; } return true; }
        if (isA && ai !== -1) { if (dir === 'left') return focusEl(ac[Math.max(0, ai - 1)] || f); if (dir === 'right') return focusEl(ac[Math.min(ac.length - 1, ai + 1)] || f); if (dir === 'up') { if (tl.length > 0) { focusEl(tl[tl.length - 1]); return true; } else if (wb && wb.offsetParent !== null) { focusEl(wb); toTop(); return true; } return focusEl(items[Math.max(0, idx - 1)] || f); } if (dir === 'down') { if (rc.length > 0) { var t = ai < rc.length ? ai : rc.length - 1; focusEl(rc[t]); return true; } else if (fi.length > 0) { focusEl(fi[0]); return true; } return true; } return true; }
        if (isR && ri !== -1) { if (dir === 'left') return focusEl(rc[Math.max(0, ri - 1)] || f); if (dir === 'right') return focusEl(rc[Math.min(rc.length - 1, ri + 1)] || f); if (dir === 'up') { if (ac.length > 0) { var t = ri < ac.length ? ri : ac.length - 1; focusEl(ac[t]); return true; } else if (tl.length > 0) { var t = ri < tl.length ? ri : tl.length - 1; focusEl(tl[t]); return true; } else if (wb && wb.offsetParent !== null) { focusEl(wb); toTop(); return true; } return focusEl(items[Math.max(0, idx - 1)] || f); } if (dir === 'down') { if (fi.length > 0) { focusEl(fi[0]); return true; } return true; } return true; }
        if (isW) { toTop(); if (dir === 'down') { if (tl.length > 0) { focusEl(tl[0]); return true; } else if (ac.length > 0) { focusEl(ac[0]); return true; } else if (rc.length > 0) { focusEl(rc[0]); return true; } else if (fi.length > 0) { focusEl(fi[0]); return true; } return true; } if (dir === 'left' || dir === 'right') return true; if (dir === 'up') return focusEl(bb || f); return true; }
        ...
        if (dir === 'up') { var t = items[Math.max(0, idx - 1)] || f; focusEl(t); if (t.id === 'catalog-watch-btn' || t.id === 'back-from-detail') toTop(); return true; } if (dir === 'down') { var t = items[Math.min(items.length - 1, idx + 1)] || f; focusEl(t); if (t.id === 'catalog-watch-btn' || t.id === 'back-from-detail') toTop(); return true; } return true;
    }
